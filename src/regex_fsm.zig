...
pub fn Range(comptime IntT: type) type {
    return struct {
        ...
        pub fn contains(self: @This(), point: IntT) bool {
    ...
    fn to_state(self: *const RegexState, wc: u16) u32 {
        for (self.transitions) |tr| {
            switch (tr.dtype) {
                ...
                .range => |r| if (r.contains(wc)) return tr.to,
            }
        }
        return ErrorState;
    }
                    ...
                    switch (value) { //If value point/range intersects a point/range, exclude/partition it.
                        ...
                        .unicode, .char => |uc| {
                            switch (self.list.items[mid_i]) {
                                .none => {},
                                .unicode, .char => |uc2| if (uc == uc2) return, //No duplicates
                                .range => |r2| if (r2.intersect_split(.{ .min = uc, .max = uc })) |range_split| { //Partition into smaller ranges.
                        ...
                        .range => |r| {
                            ...
                            switch (self.list.items[mid_i]) {
                                .none => {},
                                .unicode, .char => |uc2| if (r.intersect_split(.{ .min = uc2, .max = uc2 })) |range_split| {
                            ...
                            while (leftover_u) |lu| {
                                ...
                                switch (self.list.items[i_ptr]) {
                                    .none => continue,
                                    .unicode, .char => |uc2| if (lu.intersect_split(.{ .min = uc2, .max = uc2 })) |range_split| {
                                        leftover_u = range_split.u;
                                        if (range_split.l) |ll| {
                            ...
                            while (leftover_l) |ll| {
                                ...
                                switch (self.list.items[i_ptr]) {
                                    .none => continue,
                                    .unicode, .char => |uc2| if (ll.intersect_split(.{ .min = uc2, .max = uc2 })) |range_split| {
                                        leftover_l = range_split.l;
                                        if (range_split.u) |lu|
